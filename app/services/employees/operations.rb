module Employees
    module Operations
        ...
        def self.update_employee(params)
            ...
            return ServiceContract.success(employee) if employee.update(first_name: params[:first_name], last_name: params[:last_name], pictureUrl: params[:pictureUrl], job_ids: params[:job_ids], team_id: params[:team_id])
            ServiceContract.error(employee.errors.full_messages)
        end
    end
end